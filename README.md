...
כדי שמשחק יעבוד בסביבה הזו, הוא חייב לעמוד ב-21 דרישות טכניות מדויקות — תקשורת עם הפלטפורמה דרך `postMessage`, קבלת שאלות, שליחת ציונים, ניהול חיים וטיימר. הכלי הזה בנוי כדי להנחות מפתחים לעמוד בכל הדרישות האלה מהרגע הראשון, ולחסוך איטרציות ארוכות של שגיאות.
...
### שלב 2 — בדוק ותקן
גוררים את קובץ ה-HTML לוולידטור. הוא מריץ 25 בדיקות אוטומטיות ומציג ציון מיידי עם פירוט מה עבר ומה לא. אם יש כשלים — הוולידטור מייצר פרומפט תיקון ממוקד לחזור אל ה-AI. 2–4 סבבים זה נורמלי. אפשר גם לראות את המשחק בתצוגה מקדימה בגודל מובייל ישר בדף.

### שלב 3 — שלח
לאחר שהמשחק עובר וולידציה (כולל 6 בדיקות ידניות — בדיקה על מובייל, ניסיון גמר משחק, ניקיון הקונסול), מופיעה טופס שליחה. פרטי המפתח + שם המשחק → פותח אפליקציית מייל עם הכל מלא, מחכה רק לצירוף הקובץ.

---

## דרישות טכניות (21 דרישות · 25 בדיקות אוטומטיות)
...
| 15 | מצב חופשי: כל שאלה מוצגת פעם אחת בלבד; לאחר שכולן הוצגו — אין יותר overlay/דיווח לפי שאלות, מוצג טוסט/באדג' חד-פעמי, והמשחק ממשיך כארקייד טהור |
...
| 21 | תקני קוד מאובטחים ונקיים מ-CI/CD: עטיפה ב-IIFE עם `'use strict'`, רק `const`/`let` ו-`===`/`!==`, ללא `var`/`console`/`eval`/`new Function`/cookies, `addEventListener` במקום `on*`, `textContent` במקום `innerHTML` דינמי |
